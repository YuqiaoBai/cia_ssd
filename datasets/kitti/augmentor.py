...
import numpy as np
from .sampler import DataBaseSampler


class Augmentor:
    def __init__(self, root_path, cfg, cls_names):
        self.root_path = root_path
        self.cls_names = cls_names['Car']

        self.data_augmentor_queue = []

        for cur_aug, cur_cfg in cfg.items():
            cur_augmentor = getattr(self, cur_aug)(config=cur_cfg)
    ...
    def gt_transformation(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.gt_transformation, config=config)
        return data_dict
