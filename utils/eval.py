...
class MetricAP():
    def __init__(self, cfg, run_path, device='cuda', bev=False):
        ...
        file_test = run_path / 'thr{}_ncoop{}.pth'.format(self.cfg['score_threshold'], self.n_coop)
    ...
    def add_sample(self, name, pred_boxes, gt_boxes, confidences):
        if len(pred_boxes)>0:
    ...
    @torch.no_grad()
    def add_samples(self, names, preds, gts, confs):
        for i in range(len(names)):
            self.add_sample(names[i], preds[i].float(), gts[i].float(), confs[i])
    ...
    def save_detections(self):
        ...
        torch.save(dict_detections, str(self.run_path / 'thr{}_ncoop{}.pth'
                                        .format(self.cfg['score_threshold'], self.n_coop)))
        self.has_test_detections = True

    def cal_avg_pos_iou_score(self):
        pass
