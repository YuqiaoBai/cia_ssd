...
class MultiGroupHead(nn.Module):
    def __init__(self, cfg, pc_range):
        ...
        self.num_anchor_per_loc = 2 * num_classes # 2
        ...
        self.tasks = Head(self.in_channels, num_pred, num_cls, use_dir=self.use_direction_classifier,
    ...
    def loss(self, batch_dict, **kwargs):
        batch_anchors = batch_dict["anchors"]             # (batch_size, 70400, 7)
        ...
        labels = batch_dict["labels"]          # cls_labels: [batch_size, 70400], elem in [-1, 0, 1].
        reg_targets = batch_dict["reg_targets"] # reg_labels: [batch_size, 70400, 8].
        cls_weights, reg_weights, cared = self.prepare_loss_weights(labels,
                                          loss_norm=self.loss_norm, dtype=torch.float32,) # all: [batch_size, 70400]
        ...
        box_preds = box_preds.view(batch_size, -1, self.box_coder.code_size)             # [batch_size, 200, 176, 14] -> [batch_size, 70400, 7].
        cls_preds = cls_preds.view(batch_size, -1, self.num_classes)  # [batch_size, 70400] -> [batch_size, 70400, 1].
        ...
        if self.use_direction_classifier:   # True
            ...
            dir_logits = preds_dict["dir_cls_preds"].view(batch_size_device, -1, 2)     # [8, 70400, 2], WeightedSoftmaxClassificationLoss.
    ...
    def post_processing(self, batch_data, test_cfg):
        ...
        detections = self.get_task_detections(self.num_classes, # num_class_with_bg
                                              test_cfg,
    ...
    def get_task_detections(self, num_class_with_bg, test_cfg, batch_cls_preds, batch_reg_preds, batch_dir_preds=None,
                            batch_iou_preds=None, batch_gt_boxes=None, batch_anchors=None, batch_anchors_mask=None):
        predictions_dicts = []
        post_center_range = self.post_center_range
        anchors = batch_anchors[0] # Anchors in all batches are the same
        batch_zip = zip(batch_reg_preds, batch_cls_preds, batch_dir_preds, batch_iou_preds,
                        batch_gt_boxes, batch_anchors_mask)
        for box_preds, cls_preds, dir_preds, iou_preds, gt_boxes, a_mask in batch_zip:
            # get reg and cls predictions
            box_preds = box_preds.float()
            cls_preds = cls_preds.float()
            box_ego = gt_boxes[(gt_boxes[:, :2]==0).all(dim=1), :]
            ...
            if top_scores.shape[0] != 0:
                ...
                box_preds = torch.cat([box_preds, box_ego], dim=0)
                top_scores = torch.cat([top_scores, torch.tensor([1.0], device=top_scores.device)], dim=0)
                top_labels = torch.cat([top_labels, torch.tensor([0], device=top_labels.device)], dim=0)
                if self.use_direction_classifier:
                    dir_labels = torch.cat([dir_labels, torch.tensor([0], device=dir_labels.device)],dim=0)
                if self.nms_type=='normal':
                    ...
                    anchors_for_nms = torch.cat([anchors[top_scores_keep], box_ego], dim=0)
...
class Head(nn.Module):
    def __init__(self, num_input, num_pred, num_cls, use_dir=False, pred_var=True, num_dir=0,
        ...
        self.conv_iou = nn.Conv2d(in_channels=num_input, out_channels=2, kernel_size=3,
